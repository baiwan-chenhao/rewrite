...
if __name__ == "__main__":
    from cjlearner import CJLearnerForClaude3_haiku, CJLearnerForDeepseekV3, CJLearnerForDoubao, CJLearnerForGPT4O
    from leetcode_gen_base import LANG_TYPE
    from leetcode_gen_simple import get_models, base_test

    base_test("qwen", "en", LANG_TYPE.CANGJIE, solutions)
    base_test("qwen", "en", LANG_TYPE.PYTHON, solutions)
    base_test("qwen", "en", LANG_TYPE.SCALA, solutions)
    ...
    # # solution = Solution1()
    # # print(solution.test(rewrite, LANG_TYPE.SCALA, exp_dir, iter=0, retest=True))
    # for solution in solutions(30):
    #     print(solution)
    #     print(solution.test(rewrite, lang_type, exp_dir, iter=2, retest=False))  # 若无改动code_gen,这里就是False.
    # notify("runing done.")
...
# language = "chinese"
#
# from agent_learning import reduce_parameters
# params = reduce_parameters(model_name=model_name, language=language, is_learning=True, infra="qwen")
# saving_path = reduce_parameters(model_name=model_name, language=language, is_learning=False, infra="qwen")
# rewrite = RewriteV1(**params, debug=True)
# solution = Solution1()
# print(solution.test(rewrite, LANG_TYPE.CANGJIE, saving_path, rewrite=True))

# todo: count the token used in code gen. the retrieve/text note/text ratio.
# qwen不能续写。也不能stop。
# deepseek学得好，但是由于qwen不支持续写和stop生成代码一塌糊涂。明天研究研究代码生成。
