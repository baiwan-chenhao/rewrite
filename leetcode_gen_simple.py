...
if __name__ == '__main__':
    ...
    # base_test("qwen", "en", LANG_TYPE.PYTHON)
    # base_test("qwen", "en", LANG_TYPE.SCALA)"doubao", "gpt-4o", "qwen",

    # todo : baseline should change model_name.

    # compare_with_baseline("claude", "en")

    # solution = Solution1()
    # print(solution.test(rewrite, LANG_TYPE.CANGJIE, exp_dir, iter=1))

    # from agent_learning import reduce_parameters
    # from rewriteV1 import RewriteV1

    # # rewriteV1
    # model_name = "deepseek-r1"
    # language = "chinese"
    # infra = "byte"
    # params = reduce_parameters(model_name=model_name, language=language, is_learning=True, infra=infra)
    # saving_path = reduce_parameters(model_name=model_name, language=language, is_learning=False, infra=infra)
    # rewrite = RewriteV1(**params, debug=True)
    # solution = Solution3()
    # print(solution.test(rewrite, LANG_TYPE.CANGJIE, saving_path, rewrite=True))

    # 问题都出在collection的强制类型上面。
    ...
    # from rewrite0114_qwen import CangjieLearningAgent
    #
    # client = OpenAI(**OPENAI_KEY["byte"])
    # model_name = "doubao-1.5-pro-256k-250115_0214"
    # agent = CangjieLearningAgent(client, model_name=model_name, load_path=f"0114-{model_name}-1.json")
    # saving_path = f"test/leet-code-simple-cangjie-byte-0114-{model_name}-1"
